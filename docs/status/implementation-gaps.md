...
- **Closed by:** runtime-migration plan A3 + A3b — API `/api/v1/nodes/{id}/hot-swap` now returns runner-reported `compile_ns`, `instantiate_ns`, `signal_ns`, `ack_ns`, `convergence_ns`; ACK and first-v2 convergence are marked by transform/filter/router runner loops via `HotSwapProgress` and delivered to the API through a `tokio::sync::oneshot`. Real-runtime smoke returned `{"status":"swap_converged","timeline":{"ack_ns":600541,"compile_ns":116346250,"convergence_ns":68042,"instantiate_ns":822542,"signal_ns":1208}}`. Residual: `hot_swap_phase_ns` histogram on `/metrics` is not yet exposed and remains a follow-up. Residual (overlapping-swap edge case): if a second hot-swap request arrives on the same node before the first `mark_first_v2()` fires, the runner loop overwrites `pending_swap_progress` (`crates/wafer-core/src/runner/{transform,filter,router}.rs:56`) and the first caller times out via `"hot-swap runner exited before acknowledgement"` (`crates/wafer-core/src/api/handlers.rs:203`). Same-node overlapping swaps are unusual in deployment-controlled use, but a per-node swap-serialization guard would surface a clearer error.
