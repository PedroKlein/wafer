...
grep -q 'receipt: /var/lib/kuiper/wafer-install-receipt.json' <<<"$install_output"
...
grep -q '"protocolVersion": "3.1.1"' <<<"$seed_output"
grep -q '"qos": 1' <<<"$seed_output"
grep -q '"sendSingle": true' <<<"$seed_output"
