...
3. **Update PLAN.md** — mark session done, update future session context, add cross-session dependencies
4. **Write decision document** — `docs/decisions/YYYY-MM-DD-[topic].md` with full rationale, code sketches, sources consulted
...
The output `docs/decisions/*.md` should contain:
- **Date, status, scope, dependencies** (header)
- **Context** — why this session exists, what feeds into it
- **Amendments** (if any) — what changes from prior sessions
- **Each decision** with: the choice, rationale (brief), code sketch, interactions
- **Summary** — type hierarchy, data flow, or whatever the core mental model is
- **Future work** — what was explicitly scoped out and why
- **Sources consulted** — table of every source that informed a decision
